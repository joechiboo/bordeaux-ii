export const meetingRecords = [
	{
		...
		reports: [
			...
			'10月15日：公佈10月例會會議記錄。建商的水電包商到場檢查G6-3F及E3-2F排水孔溢水狀況。在EF棟公廁門前天花板畫出維修孔位置圖給建商',
			'10月17日：龔組長會同聯恆人員會勘地下室車道LCD顯示系統裝設位置。命氏處理好AB棟上水塔高低水位警示問題。提供具有溫度計的掛鐘產品比較表給管委會參考。麗池務業法務處主管在下午5點到社區對保全及社區主管實施教育訓練',
			...
			'10月23日：與B5-6F設計師入屋查看裝潢情況。經查拆除的紅磚牆都已在原址砌好紅磚牆，但臥室與陽台間的牆已拆除(牆上留有已切除的鋼筋痕跡)。經與會委員表決，決議:12票同意寄存證信函給B5-6F區權人，要求暫停施工',
		...
		issues: [
			...
			'議題五：郵件收發及委託物品寄放櫃台管理辦法。說明：以10月2日例會提出的範本討論修改及決議。決議：1.針對『肆、管理中心不代收以下物件：六、不受理團購物品代收、寄放』：2票同意整條刪除；11票同意修改成『六、不受理同住戶、同商品超過5箱(件)物品代收、寄放』。2.11票同意，增列『*柒、物品領取方式：住戶須以手機條碼或磁扣領取信件、包裹等物品』。3.針對存放期限10日：3票同意改為7日；6票同意維持10日，並修訂為『超過10日未領，將該戶列入拒絕服務對象』',
			'議題六：門禁安全管理辦法。說明：以褚監委提供的『樂非莊園_門禁安全管理辦法』為討論藍本。決議：1.『伍、人員出入動線規劃』部份的二、三項順序對調。2.刪掉『柒、識別證之種類規劃』',
			'議題七：公共空間堆放雜物問題討論。說明：目前的處置方式是貼勸導單，進一步的處置方式尚待確立。經主委詢問法顧答覆：無主物在無主雜物上貼勸導單三天後若仍未清除則在拍照存檔後逕行清除；有主物貼勸導單6個月後若仍未清除則在拍照存檔後逕行清除；影響消防通道梯間的有主物貼勸導單3天後若仍未清除則在掃證拍照後通報主管機關開罰。決議：10票同意，以社區法律顧問提供的方式作為執行準則',
			...
			'臨時動議二：提議推翻管委會原先對家教室內的電源插座裝上盲蓋的決定。說明：反對如裝盲蓋及贊同加裝盲蓋的委員主張詳細討論。決議:針對此項提議，表決結果為：6票同意裝設盲蓋、6票反對裝設盲蓋。主委投下關鍵一票贊同裝設盲蓋：維持原決議，裝設盲蓋',
